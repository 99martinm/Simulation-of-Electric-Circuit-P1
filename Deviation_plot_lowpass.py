...
"""
Forsøg på nyt deviation plot for lowpass filter
"""
...
def mag(f=Frequency):
    m = 20*np.log10(1/(np.sqrt(1+0.0002**2*2**2*np.pi**2*f**2)))
    return m

def phase(f=Frequency):
    p = np.degrees(np.arctan(-0.0002*2*np.pi*f))
    return p

#Magforskel ved første frekvens:
print(Magnitude[0]-mag(10))

#Magforskel ved cutoff frequency:
print(Magnitude[634]-mag(797.9946872679769))

#Magforskel ved største frequency:
...
plt.tight_layout()
plt.legend()
#plt.savefig('DeviationExp2LowPass.png',dpi=500)
